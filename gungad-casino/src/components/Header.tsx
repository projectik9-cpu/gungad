...
interface HeaderProps {
  ...
  /** Current main view for bottom-nav highlight */
  activeTab?: string;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const onCasino = activeTab === 'game' && Boolean(activeGameId);
  ...
  return (
    <>
      {/* Compact top chrome — logo + balance (like reference apps) */}
      <header
        ref={headerRef}
        className="sticky top-0 z-[200] bg-[#0a0a0d]/98 backdrop-blur-xl border-b border-rose-900/30"
        onPointerDown={unlock}
      >
        <div className="w-full max-w-7xl mx-auto pl-2 pr-2 sm:pl-3 sm:pr-4 py-2 flex items-center justify-between gap-1.5 sm:gap-3 min-w-0">
          <div className="flex items-center gap-1.5 shrink-0 min-w-0">
            {/* Desktop: menu stays top-left */}
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
            ...
            <div className="relative flex items-center gap-1">
              ...
              {playMode === 'real' && (
                <button
                  ...
                  className="hidden sm:inline-flex px-2.5 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wide bg-gradient-to-r from-rose-600 to-rose-700 text-white whitespace-nowrap"
                >
                  {t('deposit', lang)}
                </button>
              )}
            ...
            <button
              ...
              className="relative hidden md:block rounded-full border-2 border-rose-600/60 shrink-0"
            >
      ...
      {/* Mobile bottom nav — thumb-reach like the reference */}
      <nav
        className="md:hidden fixed bottom-0 inset-x-0 z-[200] bg-[#0a0a0d]/98 backdrop-blur-xl border-t border-rose-900/40 pb-[env(safe-area-inset-bottom)]"
        onPointerDown={unlock}
      >
        <div className="flex items-stretch justify-around px-1 pt-1.5 pb-1 max-w-lg mx-auto">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-0.5 min-w-0 flex-1 py-1 ${
              onHome ? 'text-rose-400' : 'text-zinc-400'
            }`}
          >
            <Home className="w-5 h-5" />
            <span className="text-[10px] font-semibold truncate max-w-full px-0.5">{t('navHome', lang)}</span>
          </button>

          <button
            onClick={goCasino}
            className={`flex flex-col items-center justify-center gap-0.5 min-w-0 flex-1 py-1 ${
              onCasino ? 'text-rose-400' : 'text-zinc-400'
            }`}
          >
            <Dices className="w-5 h-5" />
            <span className="text-[10px] font-semibold truncate max-w-full px-0.5">{t('navCasino', lang)}</span>
          </button>

          <button
            onClick={() => { soundFx.playClick(); onOpenDeposit(); }}
            className="flex flex-col items-center justify-center gap-0.5 min-w-0 flex-1 py-1 text-zinc-400 active:text-rose-400"
          >
            <CircleDollarSign className="w-5 h-5" />
            <span className="text-[10px] font-semibold truncate max-w-full px-0.5">{t('deposit', lang)}</span>
          </button>
          ...
          >
            ...
            <span className="text-[10px] font-semibold truncate max-w-full px-0.5">{t('profile', lang)}</span>
          </button>
        </div>
      </nav>
    </>
